...
from mmdet.core import multi_apply, multiclass_nms, anchor_inside_flags, unmap
...
def levels_to_images(mlvl_tensor):
    ...
    """
    batch_size = mlvl_tensor[0].size(0) #size取出个数
    batch_list = [[] for _ in range(batch_size)]
    channels = mlvl_tensor[0].size(1)
    for t in mlvl_tensor:
        t = t.permute(0, 2, 3, 1)#换维度 (0 1 2 3 ) -> (0 2 1 3)
        t = t.view(batch_size, -1, channels).contiguous() #改变形状 
...
class PAAHead(ATSSHead):
    ...
    def loss(self,
        ...
        featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores] # 五个尺度的特征图大小
        ...
        anchor_list, valid_flag_list = self.get_anchors(
            featmap_sizes, img_metas, device=device) #生成anchor
       ...
       # print(anchor_list[0][0].shape) #(8=batch, 5=scale, 4)
        ...
        cls_scores = torch.cat(cls_scores, 0).view(-1, cls_scores[0].size(-1)) # [* , 8]

        bbox_preds = torch.cat(bbox_preds, 0).view(-1, bbox_preds[0].size(-1)) # [* , 4]

        iou_preds = torch.cat(iou_preds, 0).view(-1, iou_preds[0].size(-1))
        labels = torch.cat(reassign_labels, 0).view(-1) # num_anchors
        ...
        if num_pos: # pos_inds_flatten
            ...
            losses_bbox = self.loss_bbox(
                pos_bbox_pred,
                pos_bbox_target,
                iou_target.clamp(min=EPS), # clamp iou ->【EPS, iou】
    ...
    def get_pos_loss(self, anchors, cls_score, bbox_pred, label, label_weight,
        ...
        anchors_all_level = torch.cat(anchors, 0) #按照维度0拼接 所有的level拼接在一起
        
        pos_scores = cls_score[pos_inds] #正样本的分类分数
        pos_bbox_pred = bbox_pred[pos_inds] #正样本的预测结果
        pos_label = label[pos_inds] # 分类标签
        pos_label_weight = label_weight[pos_inds] #分类标签权重
        pos_bbox_target = bbox_target[pos_inds] #回归目标
        pos_bbox_weight = bbox_weight[pos_inds] #回归权重
        
        pos_anchors = anchors_all_level[pos_inds] #正样本的anchor
    ...
    def paa_reassign(self, pos_losses, label, label_weight, bbox_weight,
        ...
        label = label.clone()
        label_ig = label.clone()
        label_weight = label_weight.clone()
        bbox_weight = bbox_weight.clone()
        num_gt = pos_gt_inds.max() + 1
        num_level = len(anchors)
        num_anchors_each_level = [item.size(0) for item in anchors] # [56000, 14000, 3500, 875, 234]
        num_anchors_each_level.insert(0, 0) # [0, 56000, 14000, 3500, 875, 234]
       
        inds_level_interval = np.cumsum(num_anchors_each_level) # [0 56000 70000 73500 74375 74609]
        ...
        for i in range(num_level): 
            ...
            pos_level_mask.append(mask) #5个尺度的正样本的 mask 数量等于正样本数
            
        pos_inds_after_paa = [label.new_tensor([])]
        ignore_inds_after_paa = [label.new_tensor([])]
        for gt_ind in range(num_gt): # gt
            pos_inds_gmm = []
            pos_loss_gmm = []
            gt_mask = pos_gt_inds == gt_ind # 建立 gt的mask shape:
            for level in range(num_level): # 单尺度去计算
                level_mask = pos_level_mask[level] #anchor的mask
                level_gt_mask = level_mask & gt_mask
                value, topk_inds = pos_losses[level_gt_mask].topk(   #topk返回前K个值 及其索引
        ...
        # print(ignore_inds_after_paa.shape,pos_inds_after_paa.shape)
        reassign_mask = (pos_inds.unsqueeze(1) != pos_inds_after_paa).all(1) # mask == true
    ...
    def gmm_separation_scheme(self, gmm_assignment, scores, pos_inds_gmm):
       ...
       # gmm_assignment.shape == scores.shape == pos_inds_gmm.shape
        fgs = gmm_assignment == 0
        pos_inds_temp = fgs.new_tensor([], dtype=torch.long)
        ignore_inds_temp = fgs.new_tensor([], dtype=torch.long)
        #print(pos_inds_gmm, 'pos_indes')
        if fgs.nonzero().numel():
            _, pos_thr_ind = scores[fgs].topk(1) # after_sort
            pos_inds_temp = pos_inds_gmm[fgs][:pos_thr_ind + 1]
           # print(len(pos_inds_temp),  'pos_tem')
            ignore_inds_temp = pos_inds_gmm.new_tensor([])
           # ignore_inds_temp = pos_inds_gmm[fgs][pos_thr_ind + 1:]
           # print(len(ignore_inds_temp),  'ign_tem', ignore_inds_temp)
    ...
    def _get_targets_single(self,
        ...
        bbox_targets_se = torch.zeros_like(anchors)
