...
class DyDCNv2(nn.Module):
    ...
    def forward(self, x, offset, mask):
        """Forward function."""
        x = self.conv(x.contiguous(), offset.contiguous(), mask.contiguous())
        if self.with_norm:
            x = self.norm(x)
        return x
...
class DyHeadBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for level in range(len(x)):
            ...
            out=self.task_attn_module(sum_feat / summed_levels)
...
class DyHead(BaseModule):
    ...
    def forward(self, inputs):
       ...
       # outs = self.dyhead_blocks(inputs[:5])
        outs = self.dyhead_blocks(inputs)
       # outs = tuple(ou ts)+ tuple(inputs[5:])
        return tuple(outs)