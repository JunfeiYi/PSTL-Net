...
from mmdet.core import multi_apply, multiclass_nms, MaxIoUAssigner
from mmdet.core.bbox.iou_calculators import bbox_overlaps
from mmdet.models import HEADS
from mmdet.models.dense_heads import ATSSHead
from mmdet.models.losses import FinalNMSLoss
EPS = 1e-12
try:
...
class PNH(ATSSHead):
    ...
    def __init__(self,
        ...
        super(PNH, self).__init__(*args, **kwargs)

        self.nms_loss = FinalNMSLoss(pull_weight=0.1, push_weight=0.001, nms_thr=0.5,
    ...
    def loss(self,
        ...
        nms_cls_reg_targets = self.get_targets(
            anchor_list,
            valid_flag_list,
            gt_bboxes,
            img_metas,
        ...
        (n_labels, n_labels_weight, n_bboxes_target, n_bboxes_weight, n_pos_inds,
         n_pos_gt_index) = nms_cls_reg_targets
        ...
        for i in anchor_list:
            anchor_all_level.append(torch.cat(i, 0))

        sigmoid_cls_scores = []
        decode_bbox_preds = []

        for cls_score, anchor, n_pos_ind, bbox_pred, n_iou_preds in zip(cls_scores, anchor_all_level, n_pos_inds, bbox_preds, iou_preds):
            n_iou_preds = n_iou_preds.sigmoid()
            # k = cls_score.sigmoid()
            cls = cls_score.sigmoid() * n_iou_preds
            sigmoid_cls_scores.append(cls.sqrt())
            pos_anchors = anchor[n_pos_ind]
            pos_bbox_pred = bbox_pred[n_pos_ind]
            decode_bbox_preds.append(self.bbox_coder.decode(pos_anchors, pos_bbox_pred))


        nms_loss = self.nms_loss(n_pos_inds, n_pos_gt_index, gt_bboxes, decode_bbox_preds, sigmoid_cls_scores, gt_labels)
        # paa_loss
        self.assigner = MaxIoUAssigner(pos_iou_thr=0.1, neg_iou_thr=0.1,
        ...
        return dict(
            loss_cls=losses_cls, loss_bbox=losses_bbox, loss_iou=losses_iou, loss_se = 2.0 *(nms_loss['nms_pull_loss'] + nms_loss['nms_push_loss']))
