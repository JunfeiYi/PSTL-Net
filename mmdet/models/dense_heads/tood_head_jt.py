    ...
    def forward(self, feat, avg_feat=None):
        ...
        conv_weight = weight.reshape(
            ...
            1) * self.reduction_conv.conv.weight.reshape(
                1, self.feat_channels, self.stacked_convs, self.feat_channels) # 1可以广播机制
...
class TOODHead_Jt(ATSSHead):
    ...
    def __init__(self,
        ...
        super(TOODHead_Jt, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):  # 6
            if i < self.num_dcn:
        ...
        '''
        self.cls_refine = nn.Conv2d(
            self.feat_channels,
            1,
            3,
            padding=0)
        self.reg_refine = nn.Conv2d(
            self.feat_channels,
            1,
            3,
            padding=0)
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            # task decomposition
            avg_feat = F.adaptive_avg_pool2d(feat, (1, 1)) #自适应平均池化->变成 batch channel,1,1
            cls_feat = self.cls_decomp(feat, avg_feat) #->注意力解耦分类

            reg_feat = self.reg_decomp(feat, avg_feat) #->注意力解耦回归
            #print(cls_feat.shape, reg_feat.shape, feat.shape )
            #cls_refine_feat = self.cls_refine(cls_feat).reshape(b,-1)
            #reg_refine_feat = self.reg_refine(reg_feat).reshape(b, -1)
            cls_refine_feat = F.adaptive_avg_pool2d(cls_feat, (1, 1)).reshape(b, -1)
            reg_refine_feat = F.adaptive_avg_pool2d(reg_feat, (1, 1)).reshape(b,-1)
            # cls prediction and alignment
            cls_logits = self.tood_cls(cls_feat)# 生成logits
            cls_prob = self.cls_prob_module(feat)
            cls_score = (cls_logits.sigmoid() * cls_prob.sigmoid()).sqrt() #最终cls_score
            ...
            cls_scores.append(cls_score) # cls_score -
            bbox_preds.append(bbox_pred) # bbox_pred ->reg_bbox
            cls_refine_feats.append(cls_refine_feat)
            reg_refine_feats.append(reg_refine_feat)
        return tuple(cls_scores), tuple(bbox_preds), tuple(cls_refine_feats), tuple(reg_refine_feats)
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, labels, cls_refine_feat, reg_refine_feat, label_weights,
        ...
        Kl_loss_joint = F.kl_div(cls_refine_feat.softmax(dim=-1).log(), reg_refine_feat.softmax(dim=-1), reduction='sum')
        ...
        #N = alignment_metrics.sum() + pos_bbox_weight.sum()
      #  Kl_loss_joint = Kl_loss_joint / N
        return loss_cls, loss_bbox, alignment_metrics.sum(
        ), pos_bbox_weight.sum(), Kl_loss_joint
    ...
    def loss(self,
        ...
        losses_cls, losses_bbox,\
            cls_avg_factors, bbox_avg_factors, kl_loss = multi_apply(
                self.loss_single,
                anchor_list,
                cls_scores,
                bbox_preds,
        ...
        #kl_weight = cls_avg_factor + bbox_avg_factor
       # print(kl_loss)
        #kl_loss = list(map(lambda x: x / kl_weight, kl_loss))
       # print(kl_loss)
        return dict(loss_cls=losses_cls, loss_bbox=losses_bbox, kl_loss=kl_loss)
    ...
    def get_bboxes(self,
                   ...
                   bbox_preds,
                   score_factors=None,
                   cls_refine_feats=None,
                   reg_refine_feats=None,
                   img_metas=None,
                   cfg=None,
                   rescale=False,
                   with_nms=True,
